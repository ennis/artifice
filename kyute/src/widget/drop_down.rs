...
impl<T: Data + Display> DropDown<T> {
    ...
    #[composable]
    pub fn new(cx: Cx, choices: Vec<T>, selected_index: usize) -> DropDown<T> {
        let inner = Container::new(Label::new(cx, format!("{}", choices[selected_index])))
        ...
        DropDown {
            ...
            selected_index,
            inner,
            selected_item_changed: Signal::new(cx),
        }
    }

    /// Returns whether TODO.
    #[composable]
    pub fn selected_item_changed(&self, cx: Cx) -> Option<T> {
        self.selected_item_changed.value(cx).map(|x| x.1)
    }
