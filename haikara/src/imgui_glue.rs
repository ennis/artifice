...
use artifice_gfxbase::vertex::{VertexData, VertexLayout, VertexLayoutElement, Semantic};
...
impl ImGuiRenderer {
    ...
    pub fn new(gl: &Gl, context: &mut imgui::Context) -> ImGuiRenderer {
        let vao = VertexArrayHandle::new(gl, &[&VertexLayout {
            elements: &[
                VertexLayoutElement {
                    semantic: None,
        ...
        let matrix_loc = unsafe { gl.GetUniformLocation(program.obj, b"matrix\0".as_ptr() as *const i8) };

        ImGuiRenderer { vao, font_tex, program, matrix_loc }
    }
    ...
    pub fn render(&self, gl: &Gl, ui: imgui::Ui) {
        ...
        unsafe {
            ...
            gl.Viewport(0, 0, draw_data.display_size[0] as i32, draw_data.display_size[1] as i32);
                    ...
                    imgui::DrawCmd::Elements { count, cmd_params } => unsafe {
                        gl.Scissor(
                            ...
                            ((cmd_params.clip_rect[2] - cmd_params.clip_rect[0])
                                * draw_data.framebuffer_scale[0]) as i32,
                            ((cmd_params.clip_rect[3] - cmd_params.clip_rect[1])
                                * draw_data.framebuffer_scale[1]) as i32,
                        );
